...
test_file_name = get_test_file_name('builtin05.py')
...
class TestTirpan(unittest.TestCase):
    ...
    def test_walk_var_x(self):
        node = findNode(self.ast, line=3, col=6, kind=ast.Name)
        self.assertTrue(node is not None, 'required node was not found')
        self.assertTrue(hasattr(node, 'link'), 'node has no link to type info')
        self.assertTrue(isinstance(node.link, VarTypeGraphNode), 'type is not a var')
        nodeType = node.link.nodeType
        self.assertTrue(len(nodeType) == 2 and                                                       \
                        all([isinstance(elem, TypeUnicode) and elem.value for elem in nodeType]) and \
                        any([elem.value.endswith('import03') for elem in nodeType]) and              \
                        any([elem.value.endswith('inner'   ) for elem in nodeType]),                 \
                        'wrong types calculated')
        self.assertEqual(node.link.name, 'x', 'name is not "x"')

    def test_walk_var_y(self):
        node = findNode(self.ast, line=3, col=9, kind=ast.Name)
        self.assertTrue(node is not None, 'required node was not found')
        self.assertTrue(hasattr(node, 'link'), 'node has no link to type info')
        self.assertTrue(isinstance(node.link, VarTypeGraphNode), 'type is not a var')
        nodeType = node.link.nodeType
        type1 = TypeList()
        type2 = TypeList()
        type2.add_elem(TypeUnicode(u'inner'))
        self.assertTrue(len(nodeType) == 2 and                                                       \
    ...
    def test_walk_var_z(self):
        node = findNode(self.ast, line=3, col=12, kind=ast.Name)
        self.assertTrue(node is not None, 'required node was not found')
        self.assertTrue(hasattr(node, 'link'), 'node has no link to type info')
        self.assertTrue(isinstance(node.link, VarTypeGraphNode), 'type is not a var')
        nodeType = node.link.nodeType
        type1 = TypeList()
        type1.add_elem(TypeUnicode(u'__init__.py'))
        type2 = TypeList()
        type2.add_elem(TypeUnicode(u'__init__.py'))
        type2.add_elem(TypeUnicode(u'_import03.py'))
        self.assertTrue(len(nodeType) == 2 and                                                       \
                        any([type1 == elem for elem in nodeType]) and                                \
                        any([type2 == elem for elem in nodeType]),                                   \
                        'wrong types calculated')
