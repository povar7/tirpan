...
def process_product_elem(pair, args, arg_elem, starargs, stararg_elem, kwargs, kwarg_elem, attr_call, file_number):
  ...
  if key is None:
    ...
    if isinstance(func, UsualFuncDefTypeGraphNode):
      ...
      call_result = process_results(config.current_res, func.defReturn)
      if cls_instance and any(isinstance(x,TypeNone) for x in call_result):
        call_result = filter(lambda x: not isinstance(x,TypeNone), call_result)
        call_result = set(call_result + [cls_instance])
      func.templates[elem_copy].result = call_result
